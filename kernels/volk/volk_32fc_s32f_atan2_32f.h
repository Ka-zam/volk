  ...
  const __m256 sign_mask = _mm256_castsi256_ps(_mm256_set1_epi32(0x80000000));   
    ...
    for (; number < eighth_points; number++) {   
        ...
        __m256 swap_mask = _mm256_cmp_ps(
            ...
            _CMP_GT_OS);  
        __m256 input = _mm256_div_ps(
            _mm256_blendv_ps(y, x, swap_mask), 
            _mm256_blendv_ps(x, y, swap_mask) );
        __m256 result = _m256_arctan_approximation_avx2_fma(input);
        result = _mm256_blendv_ps( result, _mm256_sub_ps( _mm256_or_ps(pi_2, _mm256_and_ps(input, sign_mask)), result ), swap_mask );
