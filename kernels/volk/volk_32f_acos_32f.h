...
volk_32f_acos_32f_neon(float* bVector, const float* aVector, unsigned int num_points)
{
    ...
    for (; number < quarterPoints; number++) {
        ...
        sqrt_est = vmulq_f32(sqrt_est, vrsqrtsq_f32(vmulq_f32(sqrt_arg, sqrt_est), sqrt_est));
        ...
        for (i = 0; i < 2; i++) {
            ...
            sqrt_sum_est = vmulq_f32(sqrt_sum_est,
                                      vrsqrtsq_f32(vmulq_f32(sum, sqrt_sum_est), sqrt_sum_est));
...
volk_32f_acos_32f_neonv8(float* bVector, const float* aVector, unsigned int num_points)
{
    ...
    for (; number < quarterPoints; number++) {
        ...
        arccosine = vbslq_f32(aVal_neg, vnegq_f32(arccosine), arccosine);
