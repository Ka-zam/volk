...
#ifndef INCLUDED_volk_32f_rsqrt_32f_a_H
#define INCLUDED_volk_32f_rsqrt_32f_a_H

#ifdef LV_HAVE_GENERIC
static inline void
volk_32f_rsqrt_32f_generic(float* out, const float* in, unsigned int num_points)
{
...
static inline void
volk_32f_rsqrt_32f_a_avx(float* out, const float* in, unsigned int num_points)
{
    ...
    for (unsigned int number = 0; number < eighth_points; number++) {
        ...
        __m256 r = _mm256_rsqrt_1_avx_ps(x);
    ...
    volk_32f_rsqrt_32f_generic(out, in, num_points - done);
}
#endif /* LV_HAVE_AVX */

#endif /* INCLUDED_volk_32f_reciprocal_32f_a_H */

#ifndef INCLUDED_volk_32f_rsqrt_32f_u_H
#define INCLUDED_volk_32f_rsqrt_32f_u_H

#if LV_HAVE_AVX
#include <immintrin.h>
#include <volk/volk_avx2_fma_intrinsics.h>
static inline void
volk_32f_rsqrt_32f_u_rcp(float* out, const float* in, unsigned int num_points)
{
    const unsigned int eighth_points = num_points / 8;
    for (unsigned int number = 0; number < eighth_points; number++) {
...
#include <volk/volk_avx_intrinsics.h>
static inline void
volk_32f_rsqrt_32f_u_avx(float* out, const float* in, unsigned int num_points)
{
    const unsigned int eighth_points = num_points / 8;
    for (unsigned int number = 0; number < eighth_points; number++) {
        __m256 x = _mm256_loadu_ps(in);
